...
describe PGPool::Wrapper::NodeInfo do
  before do
    ...
    @node_info        = PGPool::Wrapper::NodeInfo.build_from_raw_data(
      @node_info_id,
  ...
  it 'only admits valid status' do
    [PGPool::Wrapper::NodeInfo::INITIALIZING,
     PGPool::Wrapper::NodeInfo::UP_NO_CONNECTIONS,
     PGPool::Wrapper::NodeInfo::UP,
     PGPool::Wrapper::NodeInfo::DOWN].each do |status|
      expect(PGPool::Wrapper::NodeInfo.new(1, 'backend1', 5432, status, 0.5).class).to be(PGPool::Wrapper::NodeInfo)
    end
  end
